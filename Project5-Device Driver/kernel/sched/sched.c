...
#define USER_SP_BASE 0xffffffffa1400000
...
void init_kernel_stack(pid_t pid, pcb_t *pcb, uint64_t argv)
{
...
void set_pcb(pid_t pid, pcb_t *pcb, task_info_t *task_info, uint64_t argv)
{
    ...
    init_kernel_stack(proc_id, pcb, argv);
...
pid_t do_spawn(task_info_t *task, uint64_t argv)
{
    pid_t pid = process_id++;
    set_pcb(pid, pcb, task, argv);
    return pid;
}
