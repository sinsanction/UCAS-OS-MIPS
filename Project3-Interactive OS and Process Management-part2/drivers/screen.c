...
int screen_cursor_x[CORE_NUM];
int screen_cursor_y[CORE_NUM];
...
void screen_write_ch(char ch)
{
    if (ch == '\n')
    {
        screen_cursor_x[core_id] = 0;
        screen_cursor_y[core_id]++;
    }
    else if (ch == 8) // backspace
    {
        if (screen_cursor_x[core_id] > 0)
        {
            screen_cursor_x[core_id]--;
            new_screen[screen_cursor_y[core_id] * SCREEN_WIDTH + screen_cursor_x[core_id]] = ' ';
        }
    }
    else if (ch == 13) // enter
    {
    ...
    {
        new_screen[screen_cursor_y[core_id] * SCREEN_WIDTH + screen_cursor_x[core_id]] = ch;
        screen_cursor_x[core_id]++;
    }
    ...
    if (screen_cursor_x[core_id] >= SCREEN_WIDTH)
    {
        screen_cursor_y[core_id]++;
        screen_cursor_x[core_id] = 0;
    }

    if (screen_cursor_y[core_id] < 0)
    {
        screen_cursor_y[core_id] = 0;
    }

    if (screen_cursor_y[core_id] >= SCREEN_HEIGHT)
    {
        screen_scroll(SCREEN_HEIGHT / 2 + 1, SCREEN_HEIGHT - 1);
        screen_cursor_x[core_id] = 0;
        screen_cursor_y[core_id] = SCREEN_HEIGHT - 1;
    }
}
...
{
    screen_cursor_x[core_id] = x;
    screen_cursor_y[core_id] = y;
}
...
void screen_reflush(void)
{
    ...
    vt100_move_cursor(screen_cursor_x[core_id], screen_cursor_y[core_id]);
}
