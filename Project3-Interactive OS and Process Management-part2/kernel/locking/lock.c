...
void do_mutex_lock_acquire(lock_id_t lock_id)
{
    ...
    lock_queue[lock_id].holder = current_running[core_id]->pid;
    current_running[core_id]->hold_lock[++(current_running[core_id]->hold_lock_count)] = lock_id;
}
...
void do_mutex_lock_release(lock_id_t lock_id)
{
    ...
    for(i=1; i<NUM_MAX_LOCK; i++){
        if(current_running[core_id]->hold_lock[i] == lock_id)
            break;
    }
    if(i == current_running[core_id]->hold_lock_count){
        (current_running[core_id]->hold_lock_count)--;
    }
    else{
        for(; i<(current_running[core_id]->hold_lock_count); i++){
            current_running[core_id]->hold_lock[i] = current_running[core_id]->hold_lock[i+1];
        }
        (current_running[core_id]->hold_lock_count)--;
    }
}
