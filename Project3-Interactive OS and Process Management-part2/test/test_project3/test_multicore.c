...
void test_multicore(void)
{
  ...
  struct task_info task_add1 = {"add1", (uint64_t)&add_task1, USER_PROCESS, 1};
  struct task_info task_add2 = {"add2", (uint64_t)&add_task2, USER_PROCESS, 1};
  struct task_info task_add3 = {"add3", (uint64_t)&add_task3, USER_PROCESS, 1};
  // single core performance
  int i;
  uint64_t singleCoreBegin = get_ticks();
  pid_t single_pid = sys_spawn(&task_add1, 0);
  ...
  pids[0] = sys_spawn(&task_add2, 1);

  pids[1] = sys_spawn(&task_add3, 0);
...
void add_task1()
{
  ...
  //printf("> [TASK] add1: core: %d", coreid);
...
void add_task2()
{
  ...
  //uint32_t coreid = get_cpu_id();
  //sys_move_cursor(0, print_location);
  //printf("> [TASK] add2: core: %d", coreid);
...
void add_task3()
{
  ...
  //uint32_t coreid = get_cpu_id();
  //sys_move_cursor(0, print_location);
  //printf("> [TASK] add3: core: %d", coreid);
