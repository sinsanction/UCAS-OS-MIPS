...
lock_id_t mutex_lock_id1 = 3;
lock_id_t mutex_lock_id2 = 7;
...
void ready_to_exit_task()
{
    ...
    mutex_lock_acquire(mutex_lock_id1);
    mutex_lock_acquire(mutex_lock_id2);
    ...
    for (i = 0; i < 5000; i++)
    {
        ...
        printf("> [TASK] I am task with pid %d, I have acquired two mutex lock. (%d)", current_running[coreid]->pid, i++);
