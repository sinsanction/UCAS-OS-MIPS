...
#define SHARE_VADDR_BASE 0xffffffffa3000000

SPF_t share_pf[NUM_SHARE_PF];
...
void share_memory_initial(void)
{
    ...
    for(i=0; i<NUM_SHARE_PF; i++){
        share_pf[i].setup = 0;
        share_pf[i].VPN2 = (SHARE_VADDR_BASE + i * PAGE_SIZE);
...
uint64_t do_shmdt(uint64_t addr)
{
    ...
    for(i=0; i<NUM_SHARE_PF; i++){
        if(share_pf[i].VPN2 == addr){
            share_pf[i].setup--;
            if(share_pf[i].setup == 0){
                memset((void*)(share_pf[i].VPN2), 0, PAGE_SIZE);
            }
            return 0;
        }
    }
