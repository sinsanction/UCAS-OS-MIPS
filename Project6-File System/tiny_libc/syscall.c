...
int sys_mkfs(void)
{
    return invoke_syscall(SYSCALL_FS_INIT, 0, 0, 0);
}

int sys_print_fs(void)
{
    return invoke_syscall(SYSCALL_FS_PRINT, 0, 0, 0);
}

void sys_clrfs(void)
{
    invoke_syscall(SYSCALL_FS_CLEAR, 0, 0, 0);
}

int sys_mkdir(char *name)
{
    return invoke_syscall(SYSCALL_FS_MKDIR, (uint64_t)name, 0, 0);
}

int sys_readdir(char *name)
{
    return invoke_syscall(SYSCALL_FS_READDIR, (uint64_t)name, 0, 0);
}

int sys_enterdir(char *name, char *remind_str)
{
    return invoke_syscall(SYSCALL_FS_ENTERDIR, (uint64_t)name, (uint64_t)remind_str, 0);
}

int sys_rmdir(char *name)
{
    return invoke_syscall(SYSCALL_FS_RMDIR, (uint64_t)name, 0, 0);
}

int sys_mknod(char *name)
{
    return invoke_syscall(SYSCALL_FS_MKNOD, (uint64_t)name, 0, 0);
}

int sys_fopen(char *name, uint32_t access)
{
    return invoke_syscall(SYSCALL_FS_OPEN, (uint64_t)name, access, 0);
}

int sys_fwrite(uint32_t fd, char *buff, uint32_t size)
{
    return invoke_syscall(SYSCALL_FS_WRITE, fd, (uint64_t)buff, size);
}

int sys_cat(char *name)
{
    return invoke_syscall(SYSCALL_FS_CAT, (uint64_t)name, 0, 0);
}

int sys_fread(uint32_t fd, char *buff, uint32_t size)
{
    return invoke_syscall(SYSCALL_FS_READ, fd, (uint64_t)buff, size);
}

int sys_close(uint32_t fd)
{
    return invoke_syscall(SYSCALL_FS_CLOSE, fd, 0, 0);
}

int sys_hard_link(char *src_name, char *dest_name)
{
    return invoke_syscall(SYSCALL_FS_HLINK, (uint64_t)src_name, (uint64_t)dest_name, 0);
}

int sys_soft_link(char *src_name, char *dest_name)
{
    return invoke_syscall(SYSCALL_FS_SLINK, (uint64_t)src_name, (uint64_t)dest_name, 0);
}
